// app/my-account/MyAccountClient.tsx
"use client";
...
export default function MyAccountClient({ initialUser }: MyAccountClientProps) {
  ...
  // When profile data arrives, populate the form
  useEffect(() => {
    if (!profileData) return;

    const customer = profileData.customer;
    const user = profileData.user as {
      name?: string;
      email?: string;
      first_name?: string;
      last_name?: string;
  ...
  const onSubmit = async (data: AccountFormData) => {
    try {
      ...
      // Change password if both current + new are provided
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="pt-4">
                <button
                  ...
                  className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-teal-600 hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 disabled:opacity-60"
                >
